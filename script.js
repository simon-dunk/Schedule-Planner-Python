...
function loadClassDatabase() {
    const savedDatabase = localStorage.getItem('classDatabase');
    if (savedDatabase) {
        try {
            const csvData = savedDatabase;
            parseClassDatabaseCSV(csvData);
            updateClassDatabaseDropdown();
        } catch (error) {
            console.error('Error loading class database:', error);
        }
    }
}

function saveClassDatabase() {
    const csvData = generateClassDatabaseCSV();
    localStorage.setItem('classDatabase', csvData);
}

function generateClassDatabaseCSV() {
    const headers = ['Name', 'Instructor', 'Days', 'Time'];
    const rows = [headers.join(',')];
    
    Object.values(classDatabase).forEach(classItem => {
        const row = [
...
function parseClassDatabaseCSV(csvText) {
    const lines = csvText.trim().split('\n');
    if (lines.length < 2) return;
    
    classDatabase = {}; // Clear existing database
    ...
    dataLines.forEach((line) => {
        try {
            ...
            if (row.length >= 4) {
                const [name, instructor, daysStr, time] = row;
                if (name && daysStr && time && isValidTimeFormat(time)) {
                    const parsedDays = parseDaysFromCSV(daysStr);
                    if (parsedDays.length > 0) {
                        const dbId = `db_${Object.keys(classDatabase).length + 1}`;
                        classDatabase[dbId] = {
                            id: dbId,
                            name: name,
            ...
            console.error('Error parsing database row:', error);
        }
    });
}

function updateClassDatabaseDropdown() {
    const dropdown = document.getElementById('classDatabaseSelect');
    if (!dropdown) return;
    
    // Clear existing options except the first one
    dropdown.innerHTML = '<option value="">Select a class from database...</option>';
    
    Object.values(classDatabase).forEach(classItem => {
        const option = document.createElement('option');
        option.value = classItem.id;
        option.textContent = `${classItem.name} - ${classItem.time}`;
        dropdown.appendChild(option);
    });
}

function addClassFromDatabase() {
    const dropdown = document.getElementById('classDatabaseSelect');
    const selectedId = dropdown.value;
    
    if (!selectedId || !classDatabase[selectedId]) {
        alert('Please select a class from the database');
        return;
    }
    
    const dbClass = classDatabase[selectedId];
    ...
    scheduleData.classes[classId] = {
        ...
        days: [...dbClass.days], // Create a copy of the days array
        time: dbClass.time,
        type: 'class'
    };
    ...
    input.onchange = function(event) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                try {
                    parseClassDatabaseCSV(e.target.result);
                    saveClassDatabase();
                    updateClassDatabaseDropdown();
                    alert(`Class database updated! ${Object.keys(classDatabase).length} classes loaded.`);
                } catch (error) {
                    alert('Error reading class database CSV: ' + error.message);
                }
            };
...
function downloadClassDatabase() {
    if (Object.keys(classDatabase).length === 0) {
        alert('Class database is empty. Add some classes first.');
        return;
    }
    ...
    if (link.download !== undefined) {
        ...
        link.setAttribute('download', `class_database_${new Date().toISOString().split('T')[0]}.csv`);
...
function addToClassDatabase() {
    ...
    saveClassDatabase();
    updateClassDatabaseDropdown();
    alert('Class added to database!');
}

function clearClassDatabase() {
    if (confirm('Are you sure you want to clear the entire class database? This cannot be undone.')) {
        classDatabase = {};
        localStorage.removeItem('classDatabase');
        updateClassDatabaseDropdown();
        alert('Class database cleared.');
    }
}
...
function handleDrop(e) {
    ...
    // Update the data
    updateItemTimeAndDay(itemId, itemType, newHour, newDay);
    // Remove old bubble and create new one
    draggedElement.remove();
    visualizeScheduleItem(itemId, itemType);
    // Update lists
    updateClassesList();
    updateWorkList();
}
function updateItemTimeAndDay(itemId, itemType, newHour, newDay) {
    const item = itemType === 'class' ? scheduleData.classes[itemId] : scheduleData.work[itemId];
    const timeRange = parseTimeRange(item.time);
    if (!timeRange) return;
    const duration = timeRange.endMinutes - timeRange.startMinutes;
    const newStartMinutes = newHour * 60;
    const newEndMinutes = newStartMinutes + duration;
    const newStartHour = Math.floor(newStartMinutes / 60);
    const newStartMin = newStartMinutes % 60;
    const newEndHour = Math.floor(newEndMinutes / 60);
    const newEndMin = newEndMinutes % 60;
    const newTimeString = `${newStartHour}:${newStartMin.toString().padStart(2, '0')}-${newEndHour}:${newEndMin.toString().padStart(2, '0')}`;
    if (itemType === 'class') {
        // For classes, update the specific day or replace all days with the new day
        item.days = [newDay];
        item.time = newTimeString;
    } else {
        // For work, update the day and time
        item.days = [newDay];
        item.day = getDayName(newDay).toLowerCase();
        item.time = newTimeString;
    }
}
// Edit functionality
function editScheduleItem(itemId, type) {
...
function editClassItem(classItem) {
    const newName = prompt('Class Name:', classItem.name);
    if (newName === null) return;
    const newInstructor = prompt('Instructor:', classItem.instructor || '');
    if (newInstructor === null) return;
    const daysText = classItem.days.map(dayIndex => getDayName(dayIndex)).join(',');
    const newDays = prompt('Days (e.g., Mon,Wed,Fri or MWF):', daysText);
    if (newDays === null) return;
    const newTime = prompt('Time (e.g., 9:00-10:30):', classItem.time);
    if (newTime === null) return;
    // Validate inputs
    if (!newName.trim() || !newDays.trim() || !newTime.trim()) {
        alert('Please fill in at least Class Name, Days, and Time');
        return;
    }
    ...
    const parsedDays = parseDays(newDays);
    if (parsedDays.length === 0) {
        alert('Please enter valid days (e.g., Mon,Wed,Fri or MWF)');
        return;
    }
    ...
    classItem.instructor = newInstructor.trim();
    classItem.days = parsedDays;
    classItem.time = newTime.trim();
    // Remove old bubbles and re-visualize
    const bubbles = document.querySelectorAll(`[data-item-id="${classItem.id}"]`);
    bubbles.forEach(bubble => bubble.remove());
    visualizeScheduleItem(classItem.id, 'class');
    updateClassesList();
    updateSummaryStats();
}
function editWorkItem(workItem) {
    const newLocation = prompt('Location:', workItem.location);
    if (newLocation === null) return;
    const newDay = prompt('Day:', getDayName(workItem.days[0]));
    if (newDay === null) return;
    const newTime = prompt('Time (e.g., 9:00-17:00):', workItem.time);
    if (newTime === null) return;
    // Validate inputs
    if (!newLocation.trim() || !newDay.trim() || !newTime.trim()) {
    ...
    const dayIndex = getDayIndexFromName(newDay);
    if (dayIndex === -1) {
        alert('Please enter a valid day name');
        return;
    }
    // Update the item
    workItem.location = newLocation.trim();
    workItem.day = newDay.toLowerCase();
    workItem.days = [dayIndex];
    workItem.time = newTime.trim();
    // Remove old bubbles and re-visualize
    const bubbles = document.querySelectorAll(`[data-item-id="${workItem.id}"]`);
    bubbles.forEach(bubble => bubble.remove());
    visualizeScheduleItem(workItem.id, 'work');
    updateWorkList();
    updateSummaryStats();
}
...
function createScheduleBubble(item, type, totalDurationMinutes, startMinute, endMinute, hoursSpanned) {
    ...
    `;
    bubbleContent += `<button class="bubble-edit" onclick="editScheduleItem('${item.id}', '${type}')" title="Edit">✎</button>`;
